...
export const BikeSelection = () => {
  ...
  const catalogFiltersHandler = () => {
    ...
    bikeTypes.forEach((type) => queryParams.append("bicycleType", type.toUpperCase()));
    wheelD.forEach((wheel) => queryParams.append("wheelSize", wheel));
    queryParams.set("userHeight", userHeight.toString());

    navigate(`/catalog?search_term=&${queryParams.toString()}`);
    scrollToTop();
  }
  ...
  const BikeTypeItems = BIKE_TYPES.map(item => (
    <li
      key={item.name}
    >
      <Elements.BikeItemWrapper>
        <div>
          <img src={item.icon}/>
        </div>
        <DefaultButton
          style={{
            backgroundColor: bikeTypes.includes(item.value) ? theme.light.palette.lightBrown : "white",
            borderColor: bikeTypes.includes(item.value) ? theme.light.palette.lightBrown : "white",
            color: !bikeTypes.includes(item.value) ? theme.light.palette.lightBrown : "white",
          }}
          onClick={() => chooseBikeTypeHandler(item.value)}
        >
          {item.name}
        </DefaultButton>
      </Elements.BikeItemWrapper>
  ...
  const WheelItems = WHEEL_TYPES.map(item => (
    <li
      key={item.name}
    >
      <Elements.WheelItemWrapper>
        <div>{item.icon}</div>
        <DefaultButton
          style={{
            backgroundColor: wheelD.includes(item.value) ? theme.light.palette.lightBrown : "white",
            borderColor: wheelD.includes(item.value) ? theme.light.palette.lightBrown : "white",
            color: !wheelD.includes(item.value) ? theme.light.palette.lightBrown : "white",
          }}
          onClick={() => chooseWheelHandler(item.value)}
        >
          {item.name}
        </DefaultButton>
      </Elements.WheelItemWrapper>
        ...
        <Elements.SelectionBox>
          ...
          <Elements.SelectionForm>
            ...
            <Elements.BottomButtonBox>
              <Button onClick={catalogFiltersHandler}>Підібрати</Button>
            </Elements.BottomButtonBox>
          </Elements.SelectionForm>
        </Elements.SelectionBox>
      </Container>
